...
- Related files: `engine/include/ChikaEngine/Application.hpp`, `engine/include/ChikaEngine/EngineContext.hpp`, `engine/src/Application.cpp`, `engine/src/EngineContext.cpp`, `engine/Runtime/RHI/`, `engine/Runtime/Render/`, `engine/Runtime/Resource/CMakeLists.txt`, `engine/Runtime/Framework/`, `engine/Runtime/Scripts/`, `tests/`
- Status: Complete
...
- `EngineContext` 类似精简后的 Engine/GameInstance 服务容器，统一拥有核心模块并按逆序关闭。
...
- Component 的启用状态切换会在真实状态变化时调用 `OnEnable`/`OnDisable`。
...
- Result:
  ...
  - CTest 通过 2/2：固定步长、动画时间、Rigidbody 默认值、Component 生命周期、反射、UID 和 Scene Play/Edit 快照恢复均通过。
...
- 在 Scene/World 边界增加明确的 BeginPlay、EndPlay、FixedUpdate 和销毁队列阶段。
