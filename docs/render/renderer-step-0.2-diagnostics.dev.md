...
原因是 `ResourceManager::BuildDefaultVertexLayout()` 固定声明 Bone Index/Weight，而静态 Vertex Shader 不使用这些输入。Phase 1 应由 Shader Reflection 生成或校验 Vertex Input，从根源移除该警告。
