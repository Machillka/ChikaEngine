...
{
    // Forward 声明 - 实际定义在 Render/ResourceHandles.h
    using MeshHandle = class THandle<struct MeshTag>;
    using TextureHandle = class THandle<struct TextureTag>;
    using ShaderHandle = class THandle<struct ShaderTag>;
    using MaterialHandle = class THandle<struct MaterialTag>;
    using TextureCubeHandle = class THandle<struct TextureCubeTag>;
} // namespace ChikaEngine::Render

namespace ChikaEngine::Resource
{
    // 重导出句柄 (forward from Render namespace)
    using MeshHandle = Render::MeshHandle;
    using TextureHandle = Render::TextureHandle;
    using ShaderHandle = Render::ShaderHandle;
    using MaterialHandle = Render::MaterialHandle;
    using TextureCubeHandle = Render::TextureCubeHandle;
